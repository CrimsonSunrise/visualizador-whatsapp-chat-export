...
function Attachment({ fileName }: IAttachment) {
	...
	const loadAttachment = useCallback(() => {
		...
		if (!file) {
			setError(new Error(`Não foi possível encontrar "${fileName}" nos arquivos`));
			return;
		}
		if (mimeType) {
			setIsLoading(true);
		...
		setError(new Error(`Não foi possível carregar "${fileName}" pois excede 250MB`));
