	...
	return (
		<S.Container>
			{messages.length > 0 && (
				<S.P>
					<S.Info>
						{isLimited && filterMode === 'índice' && (
							<span>
								Mostrando mensagens {limits.low} a{' '}
								{Math.min(limits.high, messages.length)} (
								{renderedMessages.length} de{' '}
								{messages.length})
							</span>
						)}
						{isLimited && filterMode === 'data' && (
							<span>
								Mostrando mensagens de{' '}
								{getISODateString(startDate)} to{' '}
								{getISODateString(endDate)}
							</span>
						)}
						{!isLimited && (
							<span>Mostrando todas as {messages.length} mensagens</span>
						)}
					</S.Info>
				</S.P>
			)}
